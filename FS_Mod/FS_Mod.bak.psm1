...
function get-market{
    ...
    $data | add-member -MemberType ScriptProperty -Name "Access" -Value {$switches | where {$_.market -eq $this.mktCode -and $_.type -eq 'Access'} | select Name, EM7_ID}
    ...
    $data | add-member -MemberType ScriptMethod -name "GetFreeAccessPorts" -Value {
        ...
        $defaultProperties = @('MktCode','Dist1','Dist2','AccessSwitches','SDDist1','SDDist2','VCS','RouteTargetID','RC','UnusedPorts')
    ...
    #Default View
    $defaultProperties = @('MktCode','Dist1','Dist2','AccessSwitches','SDDist1','SDDist2','VCS','RouteTargetID','RC')
    ...
    $data | Add-Member -MemberType ScriptMethod -name GetVLANs -value {
        ...
        $this | add-member -MemberType NoteProperty -name 'VLANs' -Value $vlans
...
function Start-cPutty{
    ...
    $user = $env:USERNAME
        ...
        putty.exe -new_console -ssh $cString -pw $Pass 
} 
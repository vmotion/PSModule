...
#>
    ...
    #Credentials
    if(!$creds){$global:creds = Get-Credential -UserName $env:USERNAME -Message 'Enter Peak10MS Credentials'}
    ...
    foreach($m in $market){
        ...
        if($parent -eq '1')
            {
                $switch = $switches | where-object {$_.m.substring(0,3) -in "$($m)*"}| Sort-Object Market, Type, Name| Out-GridView -Title 'Select Switch' -PassThru
            
            }else
            {
                $switch = $switches | where-object {$_.i -in $m}| Sort-Object Type| Out-GridView -Title 'Select Switch' -PassThru
            }
        ...
        $return = foreach($sw in $switch){
            ...
            foreach($int in $ints) {
               if(!$SearchValue)
               {
                    $results.result_set.$($int.name) | select @{N="Hostname";E={@($sw.Name)}}, name, alias, @{N="Sts";E={@(Switch($_.ifoperstatus){1{"Up"}2{ "Down" }})}},  @{N="AdminStatus";E={@(Switch($_.ifAdminStatus){1{"Up"}2{ "Down" }})}}, ifAdminstatus, ifoperstatus
               }else
               {
                    $results.result_set.$($int.name) | select @{N="Hostname";E={@($sw.Name)}}, name, alias, @{N="Sts";E={@(Switch($_.ifoperstatus){1{"Up"}2{ "Down" }})}},  @{N="AdminStatus";E={@(Switch($_.ifAdminStatus){1{"Up"}2{ "Down" }})}}, ifAdminstatus, ifoperstatus| where-object {$_.alias -like "*$($searchvalue)*"} 
               }
            }$counter ++
        }
    }   
...
#show interface descriptions
    ...
    #VLAN ID
    $return | foreach {$_ | Add-Member -MemberType NoteProperty -name VLAN -value ([string]($_.alias.split(":")[2]))}
    #Status
